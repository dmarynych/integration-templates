...
const check = async () => {
  ...
  const outboundRequestParams = {
    ...
    headers: {
      ...
      'Authorization': `Basic ${process.env.authToken || config.authToken}`,
    },
    body: {
      type: 'json',
      value: {
        card: JSON.parse(inboundResult.data).senderPrimaryAccountNumber,
        cvv: JSON.parse(inboundResult.data).cardCvv2Value,
